<?php
    ...
    if (isset($_GET['ajax']) && $_GET['ajax'] === '1') {
        ...
        if (!empty($search)) {
            $search = addslashes($search); // Basic lil' safety
            $searchClause = " AND (
                stu.user_first_name LIKE '%$search%' OR
                stu.user_last_name LIKE '%$search%' OR
                stu.user_email LIKE '%$search%' OR
                CONCAT(stu.user_last_name, ', ', stu.user_first_name) LIKE '%$search%' OR
        
                inst.batch LIKE '%$search%' OR
                sch.school_acr LIKE '%$search%' OR
                pr.program_name LIKE '%$search%' OR
                CONCAT(sch.school_acr, ' – ', pr.program_name) LIKE '%$search%' OR
                inst.internship_year LIKE '%$search%' OR
                inst.internship_date_started LIKE '%$search%' OR
                inst.internship_date_ended LIKE '%$search%' OR
        
                comp.company_name LIKE '%$search%' OR
                dept.department_name LIKE '%$search%' OR
                inst.internship_job_role LIKE '%$search%' OR
                supu.user_first_name LIKE '%$search%' OR
                supu.user_last_name LIKE '%$search%' OR
                CONCAT(supu.user_first_name, ' ', supu.user_last_name) LIKE '%$search%' OR
                supu.user_email LIKE '%$search%' OR
        
                intern.intern_gender LIKE '%$search%' OR
                intern.intern_birthdate LIKE '%$search%' OR
                intern.intern_city LIKE '%$search%' OR
                intern.intern_province_or_state LIKE '%$search%' OR
                intern.intern_postal_code LIKE '%$search%' OR
                intern.intern_country LIKE '%$search%' OR
                CONCAT_WS(', ', intern.intern_city, intern.intern_province_or_state, intern.intern_postal_code, intern.intern_country) LIKE '%$search%' OR
                stu.user_date_created LIKE '%$search%' OR
                stu.user_date_updated LIKE '%$search%'
            )";
        }
        ...
        $countQuery = "
            ...
            INNER JOIN users AS stu ON intern.schooluser_id = stu.user_id
            ...
            WHERE 1=1 $searchClause $orderClause
        ";
        ...
        $query = "
            ...
            LEFT JOIN departments AS dept ON sup.department_id = dept.department_id
            LEFT JOIN companies AS comp ON dept.company_id = comp.company_id
            WHERE 1=1 $searchClause $orderClause LIMIT $limit OFFSET $offset
        ";
